...
app.add_url_rule('/search/<name>', view_func=SearchProfile.searchName)
...
#         a = Conn.toProfile(x)
#         api = Visit.fetchID(a.id_channel)

#         profile = db.session.query(User).filter(User.id == x).first()
#         profile.pic = api[6]  
#         db.session.commit()


# delete = delete(Img).where(Img.owner == 2)
# db.session.execute(delete)
# db.session.commit()


# connection = db.session.connection()
# connection.execute("ALTER TABLE user add pic text;")
# db.session.commit()

# user = User(username="sdsd")
# db.session.add(user)
# db.session.commit()
